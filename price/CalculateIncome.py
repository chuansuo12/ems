...
def calculate_income(r1, r2, t, mfi_array, depot_obj):
    ...
    for index in range(t, mfi_array.__len__(), t):
        ...
        if cur_mfi_obj.mfi_six_day_avg < low_limit:  # 当六日平均小于安全区间
            if cur_mfi_obj.cumulative_increase > 0:  # 如果累计涨幅是正的，说明上涨，赶紧买！
        ...
        if cur_mfi_obj.mfi_six_day_avg > up_limit:  # 当六日平均大于安全区间
            if cur_mfi_obj.cumulative_increase < 0:  # 当累计涨幅是负的，说明下跌，赶紧卖
                depot_obj.sell(cur_mfi_obj)
...
def __get_std(mfi_array):
    std_array = []
    for mfi_obj in mfi_array:
        std_array.append(mfi_obj.mfi)
    return numpy.std(std_array, ddof=1)  # ddof=1无偏差算标准差，即除以 N-1,若ddof=0则为带偏差标准差，即除以 N
